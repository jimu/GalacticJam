...
public class NTSCEncode_RLPRO : ScriptableRendererFeature
{
    ...
    private NTSCEncode_RLPROPass NTSCEncodeRenderPass;
    public RenderPassEvent Event = RenderPassEvent.AfterRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("Hidden/Shader/NTSCEncode_RLPRO");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        NTSCEncodeRenderPass = new NTSCEncode_RLPROPass(material);

        NTSCEncodeRenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        NTSCEncode myVolume = VolumeManager.instance.stack?.GetComponent<NTSCEncode>();
        ...
        {
            renderer.EnqueuePass(NTSCEncodeRenderPass);
        }
    }
    ...
    public class NTSCEncode_RLPROPass : ScriptableRenderPass
	{
        ...
        public NTSCEncode_RLPROPass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<NTSCEncode>();
            T += Time.deltaTime;
			material.SetFloat(TV, T);
			material.SetFloat("fade", volumeComponent.fade.value);
			material.SetFloat(BsizeV, volumeComponent.brigtness.value);
			material.SetFloat(val1V, volumeComponent.lineSpeed.value);
			material.SetFloat(val2V, volumeComponent.blur.value);
