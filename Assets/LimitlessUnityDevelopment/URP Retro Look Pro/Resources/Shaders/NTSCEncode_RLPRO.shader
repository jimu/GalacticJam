...
#pragma shader_feature ALPHA_CHANNEL
	...
	float4 CompositeSample(float2 UV) 
	{
		...
		float3 Texel0 = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, C0 ).rgb;
		float3 Texel1 = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, C1 ).rgb;
		float3 Texel2 = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, C2 ).rgb;
		float3 Texel3 = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, C3 ).rgb;
	...
	float4 NTSCCodec(float2 UV)
	{
		...
		half4 colIn = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, UV);
		float fd = 1;
