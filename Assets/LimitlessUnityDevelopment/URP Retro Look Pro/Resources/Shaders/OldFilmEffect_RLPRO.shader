...
#include "Packages/com.unity.render-pipelines.core/Runtime/Utilities/Blit.hlsl"
		...
		float4 Frag0(Varyings i) : SV_Target
	{
		...
		float3 image = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(suv.x, suv.y) ).rgb;
		...
		float4 result = float4(oldImage * vI, SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(suv.x, suv.y)).a);
		...
		result = lerp(result, SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, i.texcoord) , 1 - Fade);
		return result;
	}
