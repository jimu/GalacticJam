...
public class UltimateVignette_RLPRO : ScriptableRendererFeature
{
    ...
    private UltimateVignette_RLPROPass UltimateVignetteRenderPass;
    public RenderPassEvent Event = RenderPassEvent.AfterRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("Hidden/Shader/UltimateVignetteEffect_RLPRO");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        UltimateVignetteRenderPass = new UltimateVignette_RLPROPass(material);

        UltimateVignetteRenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        UltimateVignette myVolume = VolumeManager.instance.stack?.GetComponent<UltimateVignette>();
        ...
        {
            renderer.EnqueuePass(UltimateVignetteRenderPass);
        }
    }
    ...
    public class UltimateVignette_RLPROPass : ScriptableRenderPass
	{
        ...
        public UltimateVignette_RLPROPass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<UltimateVignette>();
            UniversalResourceData resourcesData = frameData.Get<UniversalResourceData>();
            UniversalCameraData cameraData = frameData.Get<UniversalCameraData>();
            material.DisableKeyword("VIGNETTE_CIRCLE");
			material.DisableKeyword("VIGNETTE_ROUNDEDCORNERS");
			switch (volumeComponent.vignetteShape.value)
			{
				case VignetteShape.circle:
					material.EnableKeyword("VIGNETTE_CIRCLE");
					break;
				case VignetteShape.roundedCorners:
					material.EnableKeyword("VIGNETTE_ROUNDEDCORNERS");
					break;
			}


			material.SetVector(_Params, new Vector4(volumeComponent.edgeSoftness.value * 0.01f, volumeComponent.vignetteAmount.value * 0.02f, volumeComponent.innerColorAlpha.value * 0.01f, volumeComponent.edgeBlend.value * 0.01f));
			material.SetColor(_InnerColor, volumeComponent.innerColor.value);
			material.SetVector(_Center, volumeComponent.center.value);
			material.SetVector(_Params1, new Vector2(volumeComponent.vignetteFineTune.value, 0.8f));
