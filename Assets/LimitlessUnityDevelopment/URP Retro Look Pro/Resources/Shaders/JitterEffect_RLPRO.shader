...
#pragma shader_feature VHS_JITTER_V_ON
	...
	float3 yiqDist(float2 uv, float m, float t) {
		...
		signal.x = rgb2yiq(SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(offsetX.r, uv.y)).rgb).x;
		signal.y = rgb2yiq(SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(offsetX.g, uv.y)).rgb).y;
		signal.z = rgb2yiq(SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, float2(offsetX.b, uv.y)).rgb).z;
		return signal;
	}
#endif
#if VHS_TWITCH_V_ON	
	...
	float4 Frag0(Varyings i) : SV_Target
	{
		...
		col = 
			SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, p).rgb;
		signal = rgb2yiq(col);
#endif
		...
		return lerp(colIn, half4(col, SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, p).a),fade);
	}
