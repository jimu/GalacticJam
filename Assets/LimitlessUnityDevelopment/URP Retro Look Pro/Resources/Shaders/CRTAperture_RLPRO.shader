...
#include "Packages/com.unity.render-pipelines.core/Runtime/Utilities/Blit.hlsl"
	...
	float3 TEX2D(float2 c) {
		return	pow(abs(SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, c).rgb), float3(GAMMA_INPUT, GAMMA_INPUT, GAMMA_INPUT)).xyz;
	}
	...
	float4 Frag(Varyings i) : SV_Target
	{
		...
		half4 inputTex = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, pos);
