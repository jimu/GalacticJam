...
public class Glitch1 : ScriptableRendererFeature
{
    ...
    private Glitch1Pass Glitch1RenderPass;
    public RenderPassEvent Event = RenderPassEvent.AfterRenderingPostProcessing;

    public override void Create()
    {
        m_Shader = Shader.Find("Hidden/Shader/Glitch1");
        if (m_Shader == null)
        {
            return;
        }
        material = new Material(m_Shader);
        Glitch1RenderPass = new Glitch1Pass(material);

        Glitch1RenderPass.renderPassEvent = Event;
    }

    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        LimitlessGlitch1 myVolume = VolumeManager.instance.stack?.GetComponent<LimitlessGlitch1>();
        ...
        {
            renderer.EnqueuePass(Glitch1RenderPass);
        }
    }
    ...
    public class Glitch1Pass : ScriptableRenderPass
    {
        ...
        public Glitch1Pass(Material material)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var volumeComponent = VolumeManager.instance.stack.GetComponent<LimitlessGlitch1>();

            T += Time.deltaTime;


            material.SetFloat(Strength, volumeComponent.Amount.value);

            material.SetFloat(x, volumeComponent.x.value);
            material.SetFloat(y, volumeComponent.y.value);
            material.SetFloat(angleY, volumeComponent.z.value);
            material.SetFloat(Stretch, volumeComponent.stretch.value);
            material.SetFloat(Speed, volumeComponent.speed.value);

            material.SetFloat(mR, volumeComponent.rMultiplier.value);
            material.SetFloat(mG, volumeComponent.gMultiplier.value);
            material.SetFloat(mB, volumeComponent.bMultiplier.value);
